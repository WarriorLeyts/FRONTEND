export default function emailValidation(email) {
  ...
  if (checkAt && email.includes('.')) {
    // в строке должен быть хотя бы один символ "."
    arrEmail.forEach((item) => {
      if (!validСharacters.includes(item)) {
        // Делаем проверку на наличие запрещенных символов.
        validation = false;
      }
      return validation;
    });
    arrEmail = email.split('@'); // разделяем нашу строку на две части.
    const checkDot = email[email.indexOf('.') + 1] !== '.'
      && email[email.indexOf('.') + 1] !== '-';
    const checkDash = email[email.indexOf('-') + 1] !== '-'
      && email[email.indexOf('-') + 1] !== '.';
    if (checkDot && checkDash) {
      // Необходимо проверить повторяются наши символы.
      ...
      if (localName.length >= 3 && localName.length <= 25) {
        // размер локальной имени.
        if (firstElement && lastElement) {
          ...
          if (domainEmail.length >= 5 && domainEmail.length <= 25 && OneDot) {
            // размер домена.
            ...
            if (domain.length >= 2 && domain.length <= 6) {
              // размер обычного домена
              let domainValidation = true;
              domain.split('').forEach((item) => {
                if (validСharacters.slice(0, 13).includes(item)) {
