...
import { updateUser, addSetting, updateUserInfo} from "@/store/profileSlice.js";
import formattedDate from "@/formattedDate";

export default function ProfileSettings() {
  const widgetRef = useRef(null);
  const dispatch = useDispatch();
  const { userInfo, loading, error, message } = useSelector(
    (state) => state.profile
  );
  ...
  const handleFileUpload = (fileInfo) => {
    ...
    dispatch(updateUserInfo({ avatar: fileInfo.cdnUrl, }));

    setApiData((prev) => ({
      ...prev,
      avatar: fileInfo.cdnUrl,
    }));
  };
        ...
        <div className={styles.inputs}>
          ...
          <div className="block-input">
            ...
            <input
              type="text"
              name="nickname"
              className={error ? "input inpError" : "input"}
      ...
      <div className={styles.secondSetting}>
        <div className="block-input">
          ...
          <textarea
            type="text"
            name="aboutme"
            className={`${styles.inputTextArea} input`}
      ...
      <div className={styles.thirdSetting}>
        <div className="block-input">
          ...
          <input
            ...
            name="dateOfBirth"
            className="input"
            value={formattedDate(userInfo?.dateOfBirth) || ""}
            onChange={(event) => handleChange(event)}
          />
        </div>
        <div className="block-input">
          ...
          <select
            className="input"
            name="showBirthDate"
            value={userInfo?.showBirthDate || ""}
            onChange={(event) => handleChange(event)}
          >
            <option value="showAll">Показывать всем</option>
            <option value="showSubscribers">
              Показывать только подписчикам
            </option>
